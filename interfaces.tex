...
It is expected that users of the LSST Science Platform (RSP) will be able to define an alert stream filter in the RSP environment and have it installed in the LSST Alert Filtering Service, which is separate from the RSP\footenote{RSP facilities are for analysis and queries of the LSST data products and not for continuously-running processes such as alert stream filters.}.
